...
export class AgentRegistry extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      // Initialize Redis subscriptions for agent coordination
      await this.redis.subscribe('agent:register', 'agent:heartbeat', 'agent:capabilities');
  ...
  async registerAgent(agent: Agent): Promise<void> {
    try {
      ...
      await this.redis.publish('agent:register', JSON.stringify({
        action: 'registered',
        agent: agent
      }));
  ...
  async updateAgentCapabilities(agentId: string, capabilities: string[]): Promise<void> {
    ...
    await this.redis.publish('agent:capabilities', JSON.stringify({
      agentId,
      capabilities,
      timestamp: new Date()
    }));
    ...
    for (const [agentId, agent] of this.agents) {
      if (agent.status === 'active' && agent.lastHeartbeat) {
        ...
        if (timeSinceHeartbeat > timeout) {
          ...
          await this.redis.publish('agent:health', JSON.stringify({
            agentId,
            status: 'unhealthy',
  ...
  async shutdown(): Promise<void> {
    ...
    await this.redis.unsubscribe('agent:register', 'agent:heartbeat', 'agent:capabilities');
