...
export class ConsensusEngine extends EventEmitter {
  ...
  constructor(config: ConsensusConfig) {
    ...
    this.redis = new Redis(); // For regular operations
    this.redisPubSub = new Redis(); // For pub/sub operations
  ...
  async initialize(): Promise<void> {
    try {
      // Subscribe to consensus channels using separate pub/sub connection
  ...
  private async sendAppendEntries(logEntry: LogEntry): Promise<void> {
    ...
    await this.redisPubSub.publish('nexus:consensus:append_entries', JSON.stringify(appendEntriesMessage));
  }
  ...
  private async broadcastPBFTMessage(phase: string, proposalState: ProposalState): Promise<void> {
    ...
    await this.redisPubSub.publish(`nexus:consensus:pbft_${phase}`, JSON.stringify(message));
  }
  ...
  private async broadcastToValidators(proposalState: ProposalState, validators: string[]): Promise<void> {
    ...
    await this.redisPubSub.publish('nexus:consensus:pos_proposal', JSON.stringify(message));
  }
  ...
  private async broadcastToDelegates(proposalState: ProposalState, delegates: string[]): Promise<void> {
    ...
    await this.redisPubSub.publish('nexus:consensus:dpos_proposal', JSON.stringify(message));
  }
  ...
  private resetElectionTimeout(): void {
    ...
    // TEMPORARILY DISABLED: Election mechanism to prevent Redis crash
    // TODO: Fix Redis pub/sub connection issue
    return;
  ...
  private async startElection(): Promise<void> {
    ...
    await this.redisPubSub.publish('nexus:consensus:vote_request', JSON.stringify(voteRequest));
  ...
  private async handleVoteRequest(message: any): Promise<void> {
    ...
    await this.redisPubSub.publish('nexus:consensus:vote_response', JSON.stringify(voteResponse));
  }
  ...
  private async sendHeartbeat(): Promise<void> {
    ...
    await this.redisPubSub.publish('nexus:consensus:heartbeat', JSON.stringify(heartbeat));
  }
...
export async function initializeConsensus(): Promise<void> {
